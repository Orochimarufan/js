...
window.tmisc = (function(window, document, undefined) {
	...
	var tmisc = {
		...
		newElement: function newElement(tag, attrs, children) {
			...
			if (children)
				children.forEach(self.appendChild, self);
		...
		/// Add (a) class(es) to a DOM element w/o jQuery
		...
		/// Remove (a) class(es) from a DOM element
		rmClass: function rmClass(dom, classes) {
			dom.className = dom.className.split(" ").filter(function(cls) { return this.indexOf(cls) == -1; }, classes.split(" ")).join(" ");
			return dom.className;
		}
	};
